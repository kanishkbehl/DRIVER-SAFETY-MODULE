...
# - Distraction status detection:
#     • "Texting" when phone detected + head down
#     • "Calling" when phone detected + looking left/right
#     • "None" when no significant distraction detected
...
while True:
    ...
    # Update counters for phone/head pose
    if detections['phone']:
    ...
    # Distraction logic
    if phone_frame_counter >= 5 and headpose_frame_counter >= 5:
        if head_direction == "Looking Down":
            distraction_status = "Texting"
        elif head_direction in ["Looking Left", "Looking Right"]:
            distraction_status = "Calling"
    else:
        distraction_status = "None"

    # Runtime for fatigue score
    ...
    y_offset += 40
    cv2.putText(frame, f"Distraction: {distraction_status}", (30, y_offset), font, 0.8,
                (0, 0, 255) if distraction_status != "None" else (0, 255, 0), 2)
